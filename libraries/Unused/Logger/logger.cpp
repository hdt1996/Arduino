...
namespace Logger
{
    int CharArrays::countLengthCharArr(char*** str_arr) // same as char ** str_arr[]
    {
        int max_len=0;
        int curr_len;
    ...
    char** CharArrays::build2dArraybyStrLiteral(char* arg_string, const char* delim)
    {    
        LOG_HELPER;
        int total_delims = 0;
        int max_length = 0;
        ...
        sprintf(te,"%i",total_elems+offset);
        strcpy(str_arr[0],te);
        ...
        for(int c = 0; c < strlen(arg_string); c++)
        {
            ...
            sprintf(xx,"%c",arg_string[c]);
            if(strcmp(xx,delim) == 0)
            {
                curr_index++;
            }
            else
            {
                strcat(str_arr[curr_index],xx);
            }
        }
        return str_arr;
    }
                                                            ...
                                                            
    {
        ...
        char log_str[strlen(f_string)+1+strlen(args_str)+1];
